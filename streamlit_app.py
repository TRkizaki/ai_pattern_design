...
# Main title
                                            ...
                                            st.markdown(’<h1 class="main-header">🤖 Pattern Recognition Interactive Demo</h1>’,
                                                        unsafe_allow_html=True)
                                            st.markdown(’<p style="text-align: center; font-size: 1.2rem; color: #666;">Master's Course Presentation - Decision Methods & Clustering</p>’,
                                                        unsafe_allow_html=True)
...
# Sidebar navigation
                                            ...
                                            page = st.sidebar.selectbox(
                                                “Choose Demo Section:”,
                                                [“🏠 Overview”, “🎯 Nearest Neighbor”, “📊 Bayesian Classification”,
                                                 “🎨 K-Means Clustering”, “📈 Feature Analysis”, “⚖️ Algorithm Comparison”, “🎓 Summary”]
                                            )
...
def generate_cluster_data(n_samples=300, centers=4, random_state=42):
    X, _ = make_blobs(n_samples=n_samples, centers=centers,
                      n_features=2, random_state=random_state, cluster_std=1.5)
return X

# Overview Page

if page == “🏠 Overview”:
    st.markdown(’<h2 class="section-header">📋 Presentation Overview</h2>’, unsafe_allow_html=True)
...
st.markdown('<h3 class="section-header">📊 Algorithm Quick Reference</h3>', unsafe_allow_html=True)
...
# Nearest Neighbor Page
    ...
    st.markdown(’<h2 class="section-header">🎯 Nearest Neighbor Classification</h2>’, unsafe_allow_html=True)
...
# Bayesian Classification Page
    ...
    st.markdown(’<h2 class="section-header">📊 Bayesian Decision Theory</h2>’, unsafe_allow_html=True)
...
class2_data = np.random.multivariate_normal([2 + overlap_factor * 2, 2 + overlap_factor * 2],
                                            [[1, -0.2], [-0.2, 1]], n_half)
...
# K-Means Clustering Page
    ...
    st.markdown(’<h2 class="section-header">🎨 K-Means Clustering Algorithm</h2>’, unsafe_allow_html=True)
...
# Feature Analysis Page
    ...
    st.markdown(’<h2 class="section-header">📈 Feature Dimensionality Analysis</h2>’, unsafe_allow_html=True)
...
# Algorithm Comparison Page
    ...
    st.markdown(’<h2 class="section-header">⚖️ Algorithm Performance Comparison</h2>’, unsafe_allow_html=True)
...
with col1:
    scenario = st.selectbox(
        "Choose Scenario:",
        ["Balanced Classes", "Imbalanced Classes", "High Noise", "Small Dataset", "High Dimensional"]
    )
...
if st.button("🔄 Run Comparison"):
    ...
    if scenario == "Balanced Classes":
        X_comp, y_comp = make_classification(n_samples=400, n_features=10,
                                             n_informative=5, weights=[0.5, 0.5], random_state=42)
        ...
        X_comp, y_comp = make_classification(n_samples=100, n_features=5,
                                             n_informative=3, random_state=42)
    else:  # High Dimensional
        X_comp, y_comp = make_classification(n_samples=200, n_features=50,
    ...
    for i, (name, algorithm) in enumerate(algorithms.items()):
        ...
        # Cross-validation
        cv_scores = cross_val_score(algorithm, X_comp, y_comp, cv=5, scoring='accuracy')
    ...
    if scenario == "Balanced Classes":
        st.info("🎯 All algorithms perform well with balanced data. Choice depends on interpretability and speed requirements.")
    elif scenario == "Imbalanced Classes":
        st.warning("⚠️ Be careful with accuracy metrics on imbalanced data. Consider precision, recall, and F1-score.")
    elif scenario == "High Noise":
        st.info("🔊 Higher K values in K-NN help reduce noise sensitivity. Bayesian methods are also robust.")
    elif scenario == "Small Dataset":
        st.warning("📉 With limited data, simpler models (higher K, Naive Bayes) often perform better.")
    else:  # High Dimensional
        st.error("📈 Curse of dimensionality affects K-NN significantly. Feature selection becomes crucial.")
```

# Summary Page

elif page == “🎓 Summary”:
    st.markdown(’<h2 class="section-header">🎓 Presentation Summary</h2>’, unsafe_allow_html=True)
...
# Decision flowchart
st.markdown('<h3 class="section-header">🗺️ Algorithm Selection Guide</h3>', unsafe_allow_html=True)
...
st.markdown('<h3 class="section-header">🎯 Final Recommendations</h3>', unsafe_allow_html=True)
...
st.markdown('<h3 class="section-header">🎉 Interactive Demo Complete!</h3>', unsafe_allow_html=True)
